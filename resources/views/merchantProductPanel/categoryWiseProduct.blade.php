...
<input type="hidden" id="merchentId" value="{{$merchantId}}" />
<script>
    var startAmount   = 0;
    var endAmount     = 0;
    var subCategoryId = 0;
    ...
    function loadContent(subCategoryId,startRange,endRange) {
        ...
        $.ajax({
            url  : "{{URL::to('merchantCategoryWiseProductLoad')}}",
            type : 'GET',
            dataType : 'json',
            data : {'categoryId':categoryId,'subCategoryId':subCategoryId,'startRange':startRange,'endRange':endRange,'merchantId':merchantId},
            beforeSend: function() {
                $('.loading').show();
                categoryContentDiv.empty();
            },
    ...
    function appendDataProcess(linkNumber) {
        ...
        $.each(jsonData[linkNumber], function(index, value) {
            if(frontendLang == 1){
                ...
                productName = value.product_name_lang2;
            }
            divContent += '<div class="col-md-4 col-lg-3 col-sm-6" style="margin-bottom: 30px">'+
