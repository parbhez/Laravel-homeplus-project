						...
						<div class="tab-content">
						    <div role="tabpanel" class="tab-pane active" id="viewSubCategoryDetails">
						     	<table id="subCategoryDetailsDataTable"  class="table table-bordered" width="100%">
									...
									<tbody>
										<?php $i=0; ?>
										@if(isset($subCategoriesDetail))
											@foreach($subCategoriesDetail as $subCategoryDetails)
											<tr>
												<td>{{++$i}}</td>
												<td>
													<span>{{$subCategoryDetails->category_name_lang1}}</span>
													<select id="categoryName{{ $subCategoryDetails->id }}" class=" form-control col-md-1 input-sm"  name="category_name_id" style="display: none;" required>
														@if(isset($categories))
															@foreach($categories as $category)
																<option value="{{$category->id}}" @if($subCategoryDetails->category_id == $category->id) selected @endif>{{$category->category_name_lang1}}</option>
															@endforeach
														@endif
													</select>
												</td>
												<td>
													<span>{{$subCategoryDetails->sub_category_name_lang1}}</span>
													<select id="subCategoryName{{ $subCategoryDetails->id }}" class=" form-control col-md-1 input-sm"  name="sub_category_name_id" style="display: none;" required>
														@if(isset($subCategories))
															@foreach($subCategories as $subCategory)
																<option value="{{$subCategory->id}}" @if($subCategoryDetails->sub_category_id == $subCategory->id) selected @endif>{{$subCategory->sub_category_name_lang1}}</option>
															@endforeach
														@endif
													</select>
												</td>
												<td>
													<span>{{$subCategoryDetails->sub_sub_category_name_lang1}}</span>
													<input id="subCategoryDetailsNameEn{{ $subCategoryDetails->id }}" class=" form-control col-md-1 input-sm" required style="display: none;" type="text" name="session_name" value="{{$subCategoryDetails->sub_sub_category_name_lang1}}" />
												</td>
												<td>
													<span>{{$subCategoryDetails->sub_sub_category_name_lang2}}</span>
													<input id="subCategoryDetailsNameBn{{ $subCategoryDetails->id }}" class=" form-control col-md-1 input-sm" required style="display: none;" type="text" name="session_name" value="{{$subCategoryDetails->sub_sub_category_name_lang1}}" />
												</td>
												<td class="">
						      						@if($subCategoryDetails->status==1)
						    					...
						    					<td>
						        					<a href="javascript:;" class="btn-edit btn btn-primary btn-xs" id="editTitle_{{ $subCategoryDetails->id }}"><span class="fa fa-edit">&nbsp;{{ trans('common.edit') }}</span></a>
						        					<a href="javascript:;" style="display:none;" class="btn-update btn btn-primary btn-xs" id="upTitle_{{ $subCategoryDetails->id }}"><span class="fa fa-save">&nbsp;{{ trans('common.save') }}</span></a>
						        					&nbsp; | &nbsp;
						        					<a href="javascript:;" class="btn-refresh btn btn-info btn-xs" id="refresh_{{ $subCategoryDetails->id }}"><span class="fa fa-refresh" title="Refresh"></span></a>
						                  			&nbsp; | &nbsp;
						                  			@if($subCategoryDetails->status==1)
						        					 <a href="javascript:;" class="btn btn-warning btn-xs" onclick="inactiveConfirm('{{ $subCategoryDetails->id }}')" id="categoryDetailsId{{ $subCategoryDetails->id }}"><span class="fa fa-ban">&nbsp;{{ trans('common.inactive') }}</span></a>
						                  			@else
						                    		<a href="javascript:;" class="btn btn-success btn-xs" onclick="activeConfirm('{{ $subCategoryDetails->id }}')" id="categoryDetailsId{{ $subCategoryDetails->id }}"><span class="fa fa-check-square-o">{{ trans('common.active') }}</span></a>
						                  			@endif
						    					</td>
											</tr>
											@endforeach
						    ...
						    <div role="tabpanel" class="tab-pane" id="addSubCategoryDetails">
						    	{!!Form::open(array('route'=>'subCategoryDetails.post','class'=>'form-horizontal form-wrp subCetagoryDetails','files'=>true))!!}
...
//======@@ Save btn function @@========
	...
	$('.btn-update').on('click', function(){
		...
		$.ajax({
			url      : "{{URL::to('saveEditSubCategoryDetails')}}",
			type     : "GET",
			cache    : false,
			dataType : 'json',
	...
	$.ajax({
	    ...
	    success : function(data){
		    if(data.success == true){
				...
				$("#categoryDetailsId"+id).html('<a href="javascript:;" class="btn btn-success btn-xs" onclick="activeConfirm('+id+')" id="categoryDetailsId'+id+'"><span class="fa fa-check-square-o">&nbsp;{{ trans("common.active") }}</span></a>'); //inactive hide
	  	...
	  	success : function(data){
		  	if(data.success == true){
				...
				info_suc.delay(3000).slideUp(300);
			 	...
			 	$("#categoryDetailsId"+id).html('<a href="javascript:;" class="btn btn-warning btn-xs" onclick="inactiveConfirm('+id+')" id="categoryDetailsId'+id+'"><span class="fa fa-ban">&nbsp;{{ trans("common.inactive") }}</span></a>'); //active hide
