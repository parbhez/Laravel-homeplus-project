...
@section('content')
                        ...
                        <div class="block-content-inner">
                            ...
                            <div class="tab-content">
                                        ...
                                        @if(isset($sliders))
                                            @foreach($sliders as $sliders)
                                                <tr>
                                                    ...
                                                    <td class="">
                                                        <img class=""
                                                             src="{{URL::to('public/images/photoSlider').'/'.$sliders->image_path}}"
                                                             width="150px" height="50px">
                                                    </td>
                                                    <td class="">
                                                        <span>{{$sliders->category_name_lang1}}</span>
                                                    </td>
                                                    <td class="">
                                                        <span>{{$sliders->sub_category_name_lang1}}</span>
                                                    </td>
                                                    <td class="">
                                                        <span>{{$sliders->image_caption}}</span>
                                                    </td>
                                                    <td class="">
                                                        @if($sliders->status == 1)
                                                    ...
                                                    <td>
                                                        <a class="btn btn-primary btn-xs modal1" data-toggle="modal"
                                                           href="#modal"
                                                           onclick="loadModalEdit('photoSlider',{{ $sliders->photo_slider_id}},'edit')"><span
                                                                    class="fa fa-edit">Edit</span></a>
                                                        &nbsp; | &nbsp;
                                                        @if($sliders->status==1)
                                                            <a href="javascript:;" class="btn btn-warning btn-xs"
                                                               onclick="inactiveConfirm('{{ $sliders->photo_slider_id }}')"
                                                               id="menuId{{ $sliders->photo_slider_id }}"><span
                                                                        class="fa fa-ban">&nbsp;{{ trans('common.inactive') }}</span></a>
                                                        @else
                                                            <a href="javascript:;" class="btn btn-success btn-xs"
                                                               onclick="activeConfirm('{{ $sliders->photo_slider_id }}')"
                                                               id="menuId{{ $sliders->photo_slider_id }}"><span
                                                                        class="fa fa-check-square-o"> {{ trans('common.active') }}</span></a>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                                ...
                                <div role="tabpanel" class="tab-pane" id="addPhotoSlider">
                                    {!!Form::open(array('route'=>'photoSlider.post','class'=>'form-horizontal form-wrp photoSlider','files'=>true))!!}
    ...
    <script type="text/javascript">
        ...
        function inactiveConfirm(id) {
            $.ajax({
                url: "{{URL::to('inactivePhotoSlider')}}/" + id,
                type: 'GET',
                dataType: 'JSON',
                beforeSend: function () {
                    loading.show();
                ...
                success: function (data) {
                    if (data.success == true) {
                        ...
                        $("#menuId" + id).html('<a href="javascript:;" class="btn btn-success btn-xs" onclick="activeConfirm(' + id + ')" id="menuId' + id + '"><span class="fa fa-check-square-o">&nbsp;{{ trans("common.active") }}</span></a>'); //inactive hide
        ...
        function activeConfirm(id) {
            $.ajax({
                url: "{{URL::to('activePhotoSlider')}}/" + id,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    if (data.success == true) {
                        ...
                        $("#menuId" + id).html('<a href="javascript:;" class="btn btn-warning btn-xs" onclick="inactiveConfirm(' + id + ')" id="menuId' + id + '"><span class="fa fa-ban">&nbsp;{{ trans("common.inactive") }}</span></a>'); //active hide
