					...
					<!-- =======Start Page Content======= -->
						...
						<div class="tab-content">
										...
										@if(isset($admins))
											...
											<tr>
												...
												<td class="">
													...
													<input id="name{{ $admin->id }}" class=" form-control col-md-1 input-sm" required style="display: none;" type="text" name="session_name" value="{{$admin->full_name}}" />
												</td>
												<td class="">
													<span>{{$admin->email}}</span>
													<input id="email{{ $admin->id }}" class=" form-control col-md-1 input-sm" required style="display: none;" type="text" name="session_name" value="{{$admin->email}}" />
												</td>
						    					...
						    					<td>
						        					<a href="javascript:;" class="btn-edit btn btn-primary btn-xs" id="editTitle_{{ $admin->id }}"><span class="fa fa-edit">&nbsp;{{ trans('common.edit') }}</span></a>
						        					<a href="javascript:;" style="display:none;" class="btn-update btn btn-primary btn-xs" id="upTitle_{{ $admin->id }}"><span class="fa fa-save">&nbsp;{{ trans('common.save') }}</span></a>
						        					&nbsp; | &nbsp;
						        					<a href="javascript:;" class="btn-refresh btn btn-info btn-xs" id="refresh_{{ $admin->id }}"><span class="fa fa-refresh" title="Refresh"></span></a>
						                  			&nbsp; | &nbsp;
						                  			@if($admin->status==1)
						        					 <a href="javascript:;" class="btn btn-warning btn-xs" onclick="inactiveConfirm('{{ $admin->id }}')" id="adminId{{ $admin->id }}"><span class="fa fa-ban">&nbsp;{{ trans('common.inactive') }}</span></a>
						                  			@else
						                    		<a href="javascript:;" class="btn btn-success btn-xs" onclick="activeConfirm('{{ $admin->id }}')" id="adminId{{ $admin->id }}"><span class="fa fa-check-square-o">{{ trans('common.active') }}</span></a>
						                  			@endif
						    					</td>
											</tr>
											@endforeach
						    ...
						    <div role="tabpanel" class="tab-pane" id="addAdmin">
						    	{!!Form::open(array('route'=>'admin.post','class'=>'form-horizontal form-wrp adminForm','files'=>true))!!}
...
$.ajax({
  ...
  if(data.success == true){
	...
	info_suc.delay(3000).slideUp(300);
		...
		$("#adminId"+id).html('<a href="javascript:;" class="btn btn-success btn-xs" onclick="activeConfirm('+id+')" id="adminId'+id+'"><span class="fa fa-check-square-o">&nbsp;{{ trans("common.active") }}</span></a>'); //inactive hide
...
$.ajax({
  ...
  if(data.success == true){
    ...
    $("#adminId"+id).html('<a href="javascript:;" class="btn btn-warning btn-xs" onclick="inactiveConfirm('+id+')" id="adminId'+id+'"><span class="fa fa-ban">&nbsp;{{ trans("common.inactive") }}</span></a>'); //active hide
