...
@section('main_container')
    <div class="row">
                ...
                <div class="row">
                    <div class="col-md-12">
                        @if(count($subSubCategory) > 0)
                            <img style="display: none;" src="{{URL::to('public/images/subCategory').'/'.$subSubCategory->sub_sub_category_name_lang1}}" alt="">
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        <div class="product_title_and_list">
                            ...
                            <div class="sub_category_list">
                                <ul class="list-inline">
                                    <li>
                                        <p>{{trans('frontend.sub_sub_category')}}</p>
                                    </li>
                                    @if(isset($subSubCategory))
                                       
                                            <li>
                                                <a class="sub_category" id="subCategory_{{$subSubCategory->id}}" data-status="0">
                                                    @if(Session::get('frontend_lang') == 1)
                                                        {{$subSubCategory->sub_sub_category_name_lang1}}
                                                    @else
                                                        {{$subSubCategory->sub_sub_category_name_lang2}}
                                                    @endif
                                                </a>
                                            </li>
        ...
        <div class="loading" style="position: fixed; top: 50%;left: 40%; width:200px; height:200px;display: none;">
    ...
    <script>
        ...
        function loadContent(subSubCategoryId,startRange,endRange) {
            var subCategoryId = "@if(isset($subCategory)){{$subCategory->sub_category_id}}@endif";
            var merchantId = "@if(isset($merchantId)){{$merchantId}}@endif";
            if (!subCategoryId){
            ...
            $.ajax({
                url  : "{{URL::to('merchantSubSubCategoryWiseProductLoad')}}",
                type : 'GET',
                dataType : 'json',
                data : {'sub_category_id' : subCategoryId,'sub_sub_category_id' : subSubCategoryId,'start_range' : startRange,'end_range' : endRange, 'merchantId':merchantId},
                beforeSend: function() {
                    $('.loading').show();
                    subCategoryContentDiv.empty();
                },
        ...
        function appendDataProcess(linkNumber) {
            ...
            $.each(jsonData[linkNumber], function(index, value) {
                ...
                divContent += '<div class="col-md-3" style="margin-bottom: 30px">'+
                        '<div class="product_unit">'+
                        '<a href="'+baseUrl+'/singleProductView/'+value.product_id+'/'+value.slug+'" target="_blank">'+
